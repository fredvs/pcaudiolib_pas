...
var
  PlayerIndex1: cint32;
  ordir, opath, pathpcaudio: string;
 ...
 uos_InputAddDSPVolume(PlayerIndex1, In1Index, 1, 1);
  ...
  if uos_LoadLib(Pchar(FilenameEdit1.FileName), Pchar(FilenameEdit5.FileName), Pchar(FilenameEdit2.FileName),nil, nil, nil, nil) = 0 then
  begin
      hide;
      Height := 305;
 ...
 //  In1Index := uos_AddFromDevIn(PlayerIndex1);
   ...
   // writeln('In1Index := ' +inttostr(In1Index));
  ...
  Labelport1 := TfpgLabel.Create(self);
  with Labelport1 do
  begin
    Name := 'Labelport1';
    SetPosition(166, 42, 320, 15);
    FontDesc := '#Label1';
    ParentShowHint := False;
    Text := 'Folder + filename of PCAudio Library';
  end;
...
{$else}
 ...
 {$ENDIF}
  ...
  {$IFDEF Darwin}
    ...
    FilenameEdit5.FileName :=  ordir + 'lib/Linux/64bit/LibPcaudio-64.so';
  ...
  {$if defined(cpu86) and defined(linux)}    
    FilenameEdit1.FileName := ordir + 'lib/Linux/32bit/LibPortaudio-32.so';
      FilenameEdit2.FileName := ordir + 'lib/Linux/32bit/LibSndFile-32.so';
       FilenameEdit5.FileName :=  ordir + 'lib/Linux/32bit/LibPcaudio-32.so';
     FilenameEdit4.FileName := ordir + 'sound/testrecord.wav';
      {$ENDIF}
